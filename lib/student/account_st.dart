  ...
  Future<void> _loadUserData() async {
    ...
    try {
      ...
      DocumentSnapshot userDoc = await _firestore
          .collection('Users') // ★★★★★ Firestoreの「ドライバー」コレクション名に合わせてください ★★★★★
          .doc(_currentUser!.uid)
          .get();

      if (userDoc.exists) {
        _userData = userDoc.data() as Map<String, dynamic>?;
        if (_userData != null && _userData!.containsKey('schoolId')) {
          final String? companyId = _userData!['schoolId'] as String?;
          if (companyId != null && companyId.isNotEmpty) {
            // 会社IDから会社名を取得
            DocumentSnapshot companyDoc = await _firestore
                .collection('schools') // ★★★★★ Firestoreの「会社」コレクション名に合わせてください ★★★★★
                .doc(companyId)
                .get();
            if (companyDoc.exists) {
              _schoolName = (companyDoc.data() as Map<String, dynamic>?)?['name'] as String?; // ★★★★★ 会社名フィールドを確認 ★★★★★
            }
          }
        }
      }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Row(
          ...
          children: [
            FittedBox(fit: BoxFit.scaleDown,child: Text('アカウント情報', style: const TextStyle(color: Colors.white))),
