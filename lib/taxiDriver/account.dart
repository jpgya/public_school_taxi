...
class _AccountPageState extends State<AccountPage> {
  ...
  String? _companyName; // Firestoreから取得した会社名
  int? _maxNum; // Firestoreから取得した最大Nums
  ...
  Future<void> _loadUserData() async {
    ...
    try {
      ...
      DocumentSnapshot userDoc = await _firestore
          .collection('drivers') // ★★★★★ Firestoreの「ドライバー」コレクション名に合わせてください ★★★★★
          .doc(_currentUser!.uid)
          .get();

      if (userDoc.exists && userDoc.data() != null) {
        // Firestoreから取得したデータをまずMapにキャストして変数に格納
        _userData = userDoc.data() as Map<String, dynamic>;

        // --- 'maxNum' の取得 ---
        // Firestoreのnumber型は int または double の可能性があるため、numとして受け取るのが安全
        final maxNumFromDb = _userData!['maxNum'];
        if (maxNumFromDb is num) {
          _maxNum = maxNumFromDb.toInt();
        } else {
          // maxNumが存在しない、または数値型でない場合のフォールバック
          _maxNum = null; // またはデフォルト値として 0 などを設定
          debugPrint("Warning: 'maxNum' field is missing or not a number in the driver document.");
        }

        // --- 'companyName' の取得（既存の正しいロジック） ---
        if (_userData!.containsKey('companyId')) {
          final String? companyId = _userData!['companyId'] as String?;
          if (companyId != null && companyId.isNotEmpty) {
            // 会社IDから会社名を取得
            DocumentSnapshot companyDoc = await _firestore
                .collection('companys') // 会社のコレクション
                .doc(companyId)
                .get();
            if (companyDoc.exists) {
              _companyName = (companyDoc.data() as Map<String, dynamic>?)?['name'] as String?;
            }
          }
        }
      } else {
        // userDocが存在しない場合のエラーメッセージを設定
        _errorMessage = "ドライバー情報が見つかりません。";
      }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInfoRow(Icons.local_taxi_sharp, 'ナンバープレート:', _currentUser?.displayName ?? '未設定'),
            _buildInfoRow(Icons.local_taxi_sharp, '最大乗車人数:', "${_maxNum}"),
            _buildInfoRow(Icons.email_outlined, 'メールアドレス:', _currentUser?.email ?? '未登録'),
            if (_companyName != null)
              _buildInfoRow(Icons.business_outlined, '会社名:', _companyName!),
